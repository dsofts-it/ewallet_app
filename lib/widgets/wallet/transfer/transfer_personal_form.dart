...
class TransferPersonalForm extends StatefulWidget {
  const TransferPersonalForm({super.key});

  @override
  State<TransferPersonalForm> createState() => _TransferPersonalFormState();
}

class _TransferPersonalFormState extends State<TransferPersonalForm> {
  ...
  Widget build(BuildContext context) {
    final User recipient = userList[0];
    ...
    return ListView(shrinkWrap: true, padding: EdgeInsets.all(spacingUnit(2)), children: [
      ...
      AppInputBox(
        ...
        content: ListTile(
          minTileHeight: 0,
          contentPadding: EdgeInsets.symmetric(horizontal: spacingUnit(1), vertical: 0),
          leading: AvatarNetwork(radius: 20, backgroundImage: recipient.avatar),
          title: Text('Send to: ${recipient.name}', style: ThemeText.subtitle2,),
          subtitle: Text(recipient.phone, style: ThemeText.paragraph),
          trailing: IconButton(
      ...
      AppInputBox(content: Column(children: [
        ...
        Padding(
          ...
          child: Text('Minimum Transfer is ${userAccount.currencySymbol}5', style: ThemeText.caption),
        ),
      ])),
      VSpaceShort(),

      AppDropdownSearch(
        hintText: 'Search or Choose Category',
        items: categoryList,
