...
class PackageFilter extends StatelessWidget {
  ...
  final PackageFilterController controller = Get.put(PackageFilterController());

  @override
  Widget build(BuildContext context) {
    List<String> sortList = ['name', 'price', 'discount', 'vendor'];
      ...
      Obx(() => Wrap(
        ...
        children: categoryTags.map((tag) {
          return _tagButton(
            context,
            tag,
            () { controller.onUpdateCategories(tag); },
            controller.categories.contains(tag),
          );
        }).toList(),
      )),
      const VSpaceBig(),
      ...
      ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Obx(() => CheckboxListTile(
            ...
            value: controller.vendors.contains(item.name),
            onChanged: (bool? value) {
              if (value == true) {
                controller.onUpdateVendor('add', item.name);
              } else {
                controller.onUpdateVendor('remove', item.name);
              }
            },
          ));
        },
...
Future<void> showPackageFilter(BuildContext context, Function() updateFilter, List<String> tags, List<Vendor> vendors) {
